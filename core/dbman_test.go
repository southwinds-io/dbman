...
func newDb() {
	exec.Command("docker", "rm", "ilinkdb", "-f").Run()
	exec.Command("docker", "run", "--name", "ilinkdb", "-itd", "-p", "5432:5432", "-e", "POSTGRESQL_ADMIN_PASSWORD=interlink", "centos/postgresql-12-centos7").Run()
